...
let handleFunctionCall(functionName, n) =
    counter <- counter+1
    output.WriteLine("@return"+counter.ToString()+"
D=A
@SP
A=M
M=D
...
let handleFunctionDef(functionName, n) =
...
let handleLabel(className: string, x: string)=
    output.WriteLine("(" + className + "." + x + ")")

let handleUnconditional(className: string, x: string)=
    output.WriteLine("@" + className + "." + x + "
0;JMP")

let handleConditional(className: string, x: string)=
    output.WriteLine("@SP
AM=M-1
D=M
@" + className + "." + x + "
...
let Bootstrap() = 
    output.WriteLine("@256
D=A
@SP
M=D")
    handleFunctionCall("Sys.init", "0")
...
let main argv =
    ...
    let files = Directory.GetFiles(path, "*.vm")
    counter <- 0
    Bootstrap()
    for file in files do
        ...
        while not (reader.EndOfStream) do
            ...
            if words.Length > 0 then
                ...
                | "gt" -> handleGt()
                | "lt" -> handleLt()
                | "label" -> handleLabel(className, words.[1])
                | "goto" -> handleUnconditional(className, words.[1])
                | "if-goto" -> handleConditional(className, words.[1])
                | "call" -> handleFunctionCall(words.[1], words.[2])
                | "function" -> handleFunctionDef(words.[1], words.[2])
                | "return" -> handleReturn()
                | "//" -> doNothing()
                | "push" -> 
                    match words.[1] with
